...
public class Catalogo {
	...
	*/
	public boolean estaLleno() {
		return (libros.length == nroLibros);
	}
	...
	*/
	public Libro buscarPorTitulo(String titulo) {
		for(int i = 0; i < nroLibros; i++) {
			if (libros[i].titulo().equalsIgnoreCase(titulo)) {
				System.out.println("¡Lo hemos encontrado! "+libros[i]);
				return libros[i];
			}
		}
		System.out.println("No hemos encontrado el libro titulado: "+titulo);
		return null;
	}
	...
	public String toString() {
		...
		return "No hay más libros cargados en el sistema";
	}
